...
What ties the two tracks together is that the ILP solver behind the production track (CBC) *is* a Branch & Bound implementation. Studying B&B directly therefore informs the engine the production model depends on.
...
- **Larger instance sizes.** Brute force was capped at n = 25 because the curve makes the point and the larger n add no information. B&B finishes in microseconds at n = 25 and could be pushed to n = 100+ to characterize the long-tail scaling of the pruning strategy itself, which would be useful for the future LLM comparison.
